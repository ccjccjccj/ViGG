...
timestamp = "KITTI_2025-02-11_15-14-56"
...
print(f" 1.2m_inlier_num_Mean: {sum([inlier_num[i][1] / len(inlier_num) for i in range(len(inlier_num))])}")
print(f" 0.6m_inlier_num_Mean: {sum([inlier_num[i][2] / len(inlier_num) for i in range(len(inlier_num))])}")
print(f" 0.3m_inlier_num_Mean: {sum([inlier_num[i][3] / len(inlier_num) for i in range(len(inlier_num))])}")
...
print("-------- For LATEX --------")
print(f"{racc[0]:.1f} / {racc[1]:.1f} / {racc[2]:.1f} & {statistics.median(err_R):.2f} & {tacc[0]:.1f} / {tacc[1]:.1f} / {tacc[2]:.1f} & {statistics.median(err_t) * 100:.1f} & {recall / len(err_R) * 100:.1f}")